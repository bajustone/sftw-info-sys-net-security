...
The conventional approach to achieving both goals is \textit{signature-then-encryption} (StE): the sender applies a digital signature algorithm to the message and then encrypts the message together with its signature under the recipient's public key. The construction is functionally correct but pays the full computational and communication cost of each primitive. At the 1536-bit parameter regime considered representative in the late 1990s, a Schnorr-plus-ElGamal realization of StE consumes six modular exponentiations and roughly doubles the transmitted length relative to the plaintext alone~\cite{zheng1997}.

Zheng's 1997 proposal of \textit{signcryption} challenged the implicit assumption that joint confidentiality and authentication must cost the literal sum of their parts~\cite{zheng1997}. Signcryption merges signing and public-key encryption into a single algorithmic step whose total cost is strictly below that of any sequential composition of its constituent primitives. For DLP-based parameters, Zheng's original construction realizes this gain as roughly halved computation and roughly six-fold reduced communication overhead relative to Schnorr-plus-ElGamal StE.
...
The objectives of this review are threefold: (1) to trace the evolution of signcryption from its original proposal through its various extensions, (2) to comparatively analyze the security models and efficiency of existing schemes, and (3) to critically examine why practical deployment has not followed theoretical maturity, identifying what is needed to bridge this gap. We argue that signcryption's deployment failure is, at bottom, a \textit{security-model communication failure}: the research community never converged on a concise, practitioner-facing account of what signcryption provides that the deployed combination of ECDH and ECDSA does not. The recent constructive treatment by Badertscher et al.~\cite{badertscher2018}---which isolates insider security as the canonical notion---supplies, for the first time, a candidate answer phrased in the operational vocabulary that implementers actually use.

\section{Literature Review}

\subsection{Constructions and Efficiency}

Zheng~\cite{zheng1997} framed signcryption as a rebuttal to a tacit assumption in the field---that joint secure-plus-authenticated delivery had to pay the combined cost of its two component primitives. He formalized a signcryption scheme as a pair of algorithms $(S, U)$ that together must satisfy three requirements. \textit{Unique unsigncryptability} requires that the sender's message can be recovered without ambiguity by the intended receiver. \textit{Security} requires that confidentiality, unforgeability, and non-repudiation hold simultaneously. \textit{Efficiency} requires that the joint computational and communication cost fall strictly below that of the best StE construction using equivalent components.
...
The concrete construction is built on a family of shortened ElGamal-derived signatures (SDSS1 and SDSS2). Zheng's structural insight is that ElGamal-style signing and ElGamal-style encryption share an underlying Diffie--Hellman exchange, and that a single ephemeral exponent can simultaneously serve both operations---closing the redundancy that StE pays for. At 1536-bit parameters the resulting signcryption consumes three modular exponentiations on the sender side and one on the receiver side, as against the six that StE spends on the same parameters. Because the signature component is absorbed into the ciphertext rather than appended to it, bandwidth overhead drops by roughly an order of magnitude.
...
Zheng and Imai~\cite{zheng1998} subsequently ported signcryption to the elliptic curve setting, motivated by the growing importance of ECC for devices with tight compute and memory budgets---smart cards, wireless handsets, and early embedded sensors. The EC construction preserves the structural sharing of the original scheme while inheriting the shorter key lengths permitted by ECDLP. Empirically, their EC signcryption delivers around 58\% computational savings and a 40\% reduction in ciphertext size relative to an equivalent EC-StE baseline. They also surveyed candidate application domains in which these gains would be operationally meaningful, spanning authenticated key establishment, multicast secrecy, key recovery protocols, and lightweight transaction primitives---each representative of the resource-constrained contexts that continue to dominate signcryption's deployment story two decades later.
...
Malone-Lee~\cite{malonelee2002} proposed the first identity-based signcryption scheme using bilinear pairings on elliptic curves. The scheme consists of four algorithms---Setup, Extract, Signcrypt, and Unsigncrypt---and operates under the Bilinear Diffie--Hellman (BDH) assumption. Malone-Lee defined two security notions tailored to the IB setting: \textit{indistinguishability of identity-based signcryptions under chosen ciphertext attack} (IND-ISC-CCA) for confidentiality, and \textit{existential unforgeability of identity-based signcryptions under adaptive chosen message attack} (EF-ISC-ACMA) for unforgeability. The scheme provides non-repudiation through a verification equation that a third party can check independently. An efficiency comparison showed that the signcryption approach requires fewer pairing computations than a naive composition of separate IBE and IB signature schemes. Shortly thereafter, Libert and Quisquater~\cite{libert2003} identified a subtle but fatal leak in Malone-Lee's design: a component of the signature is transmitted unmasked alongside the ciphertext, and this component is functionally dependent on the plaintext. An adversary who can guess the message can therefore recompute and compare the signature component, confirming the guess without ever attacking the encryption layer---a classical guess-and-verify distinguisher that collapses semantic security. Libert and Quisquater's corrected schemes route all signature material through the symmetric encryption layer so that no plaintext-dependent value is exposed in clear, and they re-prove security under the Decisional Bilinear Diffie--Hellman (DBDH) assumption. This episode illustrates a recurring pattern in signcryption research: achieving the correct integration of signature and encryption components is subtle, and seemingly secure constructions can harbor vulnerabilities that only surface under careful formal analysis.

Boyen~\cite{boyen2003} proposed a more ambitious identity-based signature/encryption (IBSE) scheme that expands the security target well beyond basic confidentiality and unforgeability. Alongside IND-CCA2 confidentiality and existential unforgeability against chosen-message insider attacks, Boyen's scheme provides three additional guarantees, which he terms ``ciphertext anonymity'' (the ciphertext reveals neither sender nor recipient identity to outsiders), ``ciphertext unlinkability'' (the sender retains the ability to disavow authorship toward any specific recipient), and ``ciphertext authentication'' (only the intended recipient can verify the claimed sender's role in producing the ciphertext)~\cite{boyen2003}. The construction uses a two-layer ``sign-then-encrypt'' design with a detachable randomized signature followed by anonymous deterministic encryption. This architecture is more modular and secure than ``monolithic'' signcryption designs that combine signing and encryption into a single inseparable operation. Furthermore, the scheme naturally supports multi-recipient encryption with signature sharing, offering scalability advantages. Notably, Boyen's use of ephemeral randomization provides \textit{forward secrecy}---the property that compromise of long-term keys does not expose past communications---a security guarantee absent from Zheng's original construction and increasingly demanded by modern protocol designers.
...
The standard partitions signcryption mechanisms along two orthogonal axes. Along the algebraic axis, one family operates in multiplicative groups of finite fields---the lineage of Zheng's original SDSS1 and SDSS2 (Shortened Digital Signature Standard) constructions~\cite{zheng1997}---while the second operates on elliptic curve groups and descends from the Zheng--Imai line~\cite{zheng1998}. Along the session-key-handling axis, the standard admits two modes, distinguishing schemes in which the sender generates and ships a fresh symmetric session key inside the ciphertext (the ``key transport'' mode) from those in which sender and receiver jointly derive the session key from a Diffie--Hellman computation embedded in the protocol (the ``key agreement'' mode). For each family and each mode, ISO/IEC 29150 fixes concrete parameter profiles---minimum group sizes, approved hash families, and compatible symmetric ciphers.
